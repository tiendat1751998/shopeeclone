...
func (p *Producer) Publish(ctx context.Context, eventType string, payload interface{}) error {
	...
	msg := kafka.Message{Topic: TopicRecEvents, Key: []byte(eventType), Value: data, Headers: []kafka.Header{{Key: "service", Value: []byte(p.service)}, {Key: "event_type", Value: []byte(eventType)}}, Time: time.Now()}
	...
	observability.KafkaMessagesProduced.WithLabelValues(p.service, TopicRecEvents).Inc(); return nil
}
