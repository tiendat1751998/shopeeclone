...
const TopicNotificationEvents = "notification.events"
...
func (p *Producer) Publish(ctx context.Context, eventType string, payload interface{}) error {
	...
	msg := kafka.Message{Topic: TopicNotificationEvents, Key: []byte(eventType), Value: data, Headers: []kafka.Header{{Key: "service", Value: []byte(p.service)}, {Key: "event_type", Value: []byte(eventType)}}, Time: time.Now()}
	...
	observability.KafkaMessagesProduced.WithLabelValues(p.service, TopicNotificationEvents).Inc(); return nil
}
