...
func (r *NotificationRepository) FindByID(ctx context.Context, id string) (*domain.Notification, error) {
	var n domain.Notification; err := r.db.GetContext(ctx, &n, "SELECT * FROM notifications WHERE id = ?", id)
...
func (r *NotificationRepository) FindByUserID(ctx context.Context, userID string, offset, limit int) ([]*domain.Notification, int64, error) {
	...
	var notifs []*domain.Notification; err := r.db.SelectContext(ctx, &notifs, "SELECT * FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", userID, limit, offset)
...
func (r *TemplateRepository) FindByName(ctx context.Context, name string) (*domain.NotificationTemplate, error) {
	var t domain.NotificationTemplate; err := r.db.GetContext(ctx, &t, "SELECT * FROM notification_templates WHERE name = ? AND is_active = true", name)
...
func (r *PreferenceRepository) GetUserPreferences(ctx context.Context, userID string) ([]*domain.UserPreference, error) {
	var prefs []*domain.UserPreference; err := r.db.SelectContext(ctx, &prefs, "SELECT * FROM user_preferences WHERE user_id = ?", userID)
	return prefs, err
}
