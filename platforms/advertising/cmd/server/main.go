package main
import ("context"; "fmt"; "net/http"; "os"; "os/signal"; "syscall"; "time"; "github.com/gin-gonic/gin"; sharedRedis "github.com/shopee-clone/shopee/packages/go-shared/pkg/redis"; "github.com/shopee-clone/shopee/packages/go-shared/pkg/health"; "github.com/shopee-clone/shopee/packages/go-shared/pkg/observability"; "github.com/shopee-clone/shopee/platforms/advertising/internal/application"; "github.com/shopee-clone/shopee/platforms/advertising/internal/config"; httptransport "github.com/shopee-clone/shopee/platforms/advertising/internal/transport/http"; kafkatransport "github.com/shopee-clone/shopee/platforms/advertising/internal/transport/kafka"; "go.uber.org/zap")
var version = "1.0.0"
func main() {
	...
	adService := application.NewAdvertisingService(pub)
	hc := health.NewChecker(cfg.AppName, version, redisClient)
	gin.SetMode(gin.ReleaseMode); engine := gin.New()
	handler := httptransport.NewHandler(adService); router := httptransport.NewRouter(handler, hc); router.Setup(engine)
	...
	go func() { logger.Info("starting advertising service", zap.Int("http_port", cfg.HTTPPort)); if err := httpServer.ListenAndServe(); err != nil && err != http.ErrServerClosed { logger.Fatal("http server failed", zap.Error(err)) } }()
