...
func (r *Router) Setup(e *gin.Engine) {
	e.Use(middleware.Recovery(), middleware.ErrorHandler(), middleware.RequestID(), middleware.CORS(), middleware.OTelMiddleware("shopee-live-commerce"), observability.ObserveHTTPMetrics("shopee-live-commerce"))
	e.GET("/health", r.health.LivenessHandler()); e.GET("/ready", r.health.ReadinessHandler()); e.GET("/metrics", observability.MetricsHandler())
	api := e.Group("/api/v1")
	{ api.POST("/live", r.handler.CreateLivestream); api.POST("/live/:id/start", r.handler.StartLivestream); api.POST("/live/:id/end", r.handler.EndLivestream); api.POST("/live/:room_id/chat", r.handler.SendMessage); api.POST("/live/:room_id/reactions", r.handler.SendReaction); api.GET("/live/:id/viewers", r.handler.GetViewerCount) }
}
