package main
import ("context"; "fmt"; "net/http"; "os"; "os/signal"; "syscall"; "time"; "github.com/gin-gonic/gin"; sharedRedis "github.com/shopee-clone/shopee/packages/go-shared/pkg/redis"; "github.com/shopee-clone/shopee/packages/go-shared/pkg/health"; "github.com/shopee-clone/shopee/packages/go-shared/pkg/observability"; "github.com/shopee-clone/shopee/platforms/live-commerce/internal/application"; "github.com/shopee-clone/shopee/platforms/live-commerce/internal/config"; httptransport "github.com/shopee-clone/shopee/platforms/live-commerce/internal/transport/http"; kafkatransport "github.com/shopee-clone/shopee/platforms/live-commerce/internal/transport/kafka"; "go.uber.org/zap")
var version = "1.0.0"
func main() {
	...
	liveService := application.NewLiveCommerceService(pub)
	hc := health.NewChecker(cfg.AppName, version, redisClient)
	gin.SetMode(gin.ReleaseMode); engine := gin.New()
	handler := httptransport.NewHandler(liveService); router := httptransport.NewRouter(handler, hc); router.Setup(engine)
	...
	go func() { logger.Info("starting live commerce service", zap.Int("http_port", cfg.HTTPPort)); if err := httpServer.ListenAndServe(); err != nil && err != http.ErrServerClosed { logger.Fatal("http server failed", zap.Error(err)) } }()
