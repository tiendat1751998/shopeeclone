package main
import ("context"; "fmt"; "net/http"; "os"; "os/signal"; "syscall"; "time"; "github.com/gin-gonic/gin"; sharedRedis "github.com/shopee-clone/shopee/packages/go-shared/pkg/redis"; "github.com/shopee-clone/shopee/packages/go-shared/pkg/health"; "github.com/shopee-clone/shopee/packages/go-shared/pkg/observability"; "github.com/shopee-clone/shopee/platforms/search/internal/application"; "github.com/shopee-clone/shopee/platforms/search/internal/config"; "github.com/shopee-clone/shopee/platforms/search/internal/infrastructure/elasticsearch"; redisinfra "github.com/shopee-clone/shopee/platforms/search/internal/infrastructure/redis"; httptransport "github.com/shopee-clone/shopee/platforms/search/internal/transport/http"; kafkatransport "github.com/shopee-clone/shopee/platforms/search/internal/transport/kafka"; "go.uber.org/zap")
var version = "1.0.0"
func main() {
	...
	searchService := application.NewSearchService(esClient, redisStore, pub)
	hc := health.NewChecker(cfg.AppName, version, redisClient)
	gin.SetMode(getGinMode(cfg.AppEnv)); engine := gin.New()
	handler := httptransport.NewHandler(searchService); router := httptransport.NewRouter(handler, hc); router.Setup(engine)
	...
	go func() { logger.Info("starting search service", zap.Int("http_port", cfg.HTTPPort)); if err := httpServer.ListenAndServe(); err != nil && err != http.ErrServerClosed { logger.Fatal("http server failed", zap.Error(err)) } }()
