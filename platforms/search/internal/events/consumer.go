...
type RealtimeIndexHandler struct {
	indexFunc func(ctx context.Context, eventType EventType, data []byte) error
}

func NewRealtimeIndexHandler(indexFunc func(ctx context.Context, eventType EventType, data []byte) error) *RealtimeIndexHandler {
	return &RealtimeIndexHandler{indexFunc: indexFunc}
}

func (h *RealtimeIndexHandler) Handle(ctx context.Context, eventType EventType, data []byte) error {
	if h.indexFunc != nil {
		return h.indexFunc(ctx, eventType, data)
	}
	...
	observability.LogWithTrace(ctx).Info("realtime index event received",
