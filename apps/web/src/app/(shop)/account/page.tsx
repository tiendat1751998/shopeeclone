...
export default function AccountPage() {
  const { user, isAuthenticated, isLoading: authLoading, logout, fetchProfile } = useAuthStore();
  ...
  useEffect(() => {
    if (isAuthenticated && !fetchedRef.current) {
      ...
      fetchProfile();
      setIsLoading(true);
      ordersApi.list(1, 10).then((res) => setOrders(res.data)).catch(() => undefined).finally(() => setIsLoading(false));
    }
  }, [isAuthenticated, fetchProfile]);

  if (authLoading) return <div className="container py-16 text-center"><div className="skeleton h-8 w-48 mx-auto mb-4" /><div className="skeleton h-64 w-full max-w-2xl mx-auto" /></div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card p-4 h-fit">
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#e8e8e8]">
            <div className="w-12 h-12 rounded-full bg-[#ee4d2d] flex items-center justify-center text-white text-lg font-bold">
              {user?.display_name?.charAt(0)?.toUpperCase() || "U"}
            </div>
            <div><p className="font-medium text-sm">{user?.display_name || "User"}</p><p className="text-xs text-[#757575]">{user?.email || ""}</p></div>
          </div>
