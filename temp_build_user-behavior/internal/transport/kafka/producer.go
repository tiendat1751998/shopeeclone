...
const TopicBehaviorEvents = "user-behavior.events"
...
func (p *Producer) Publish(ctx context.Context, eventType string, payload interface{}) error {
	...
	msg := kafka.Message{Topic: TopicBehaviorEvents, Key: []byte(eventType), Value: data, Headers: []kafka.Header{{Key: "service", Value: []byte(p.service)}}, Time: time.Now()}
	...
	observability.KafkaMessagesProduced.WithLabelValues(p.service, TopicBehaviorEvents).Inc(); return nil
}
