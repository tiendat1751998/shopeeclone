...
func (r *CheckoutRepository) FindByID(ctx context.Context, id string) (*domain.Checkout, error) {
	...
	err := r.db.GetContext(ctx, &c, "SELECT * FROM checkouts WHERE id = ?", id)
...
func (r *CheckoutRepository) FindByIdempotencyKey(ctx context.Context, key string) (*domain.Checkout, error) {
	var c domain.Checkout
	err := r.db.GetContext(ctx, &c, "SELECT * FROM checkouts WHERE idempotency_key = ?", key)
	if err == sql.ErrNoRows { return nil, nil }
	return &c, err
}
...
func (r *CheckoutRepository) FindExpired(ctx context.Context, before string, limit int) ([]*domain.Checkout, error) {
	...
	err := r.db.SelectContext(ctx, &checkouts, "SELECT * FROM checkouts WHERE status IN ('pending','validating','pricing_frozen','reserving_inventory') AND expires_at < ? LIMIT ?", before, limit)
...
func (r *CheckoutStepLogRepository) FindByCheckoutID(ctx context.Context, checkoutID string) ([]*domain.CheckoutStepLog, error) {
	...
	err := r.db.SelectContext(ctx, &logs, "SELECT * FROM checkout_step_logs WHERE checkout_id = ? ORDER BY created_at ASC", checkoutID)
	return logs, err
}
...
func (r *PricingSnapshotRepository) FindByCheckoutID(ctx context.Context, checkoutID string) (*domain.PricingSnapshot, error) {
	...
	err := r.db.GetContext(ctx, &snap, "SELECT * FROM pricing_snapshots WHERE checkout_id = ?", checkoutID)
	if err == sql.ErrNoRows { return nil, nil }
	return &snap, err
}
...
func (r *ReservationOrchestrationRepository) FindByCheckoutID(ctx context.Context, checkoutID string) ([]*domain.ReservationOrchestration, error) {
	...
	err := r.db.SelectContext(ctx, &res, "SELECT * FROM reservation_orchestrations WHERE checkout_id = ?", checkoutID)
	return res, err
}
...
func (r *ReconciliationJobRepository) FindPending(ctx context.Context, limit int) ([]*domain.ReconciliationJob, error) {
	...
	err := r.db.SelectContext(ctx, &jobs, "SELECT * FROM reconciliation_jobs WHERE status = 'pending' AND next_retry_at <= NOW() ORDER BY next_retry_at ASC LIMIT ?", limit)
	return jobs, err
}
